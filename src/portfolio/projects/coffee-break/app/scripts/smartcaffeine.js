...
function smart_levelChecker()
{
	if(window.userLevelCurrent > window.userLevel)
	{
		var message = "You just leveled up. Congrats!"
		smartDesktopNotify(message);
		window.userLevel = window.userLevelCurrent;
	}
}
...
if(window.smartCaffeine == 1)
{
	//setTimeout(smart_workChecker,7200000);
	//setTimeout(smart_hydrationChecker,5400000);
	setTimeout(smart_levelChecker, 1800000);
}