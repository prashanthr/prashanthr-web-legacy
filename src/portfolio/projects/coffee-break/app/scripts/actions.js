...
function getTimedBreakFromUser()
{
  showTimedBreakPromptDialog();
}

function timedBreakCallback(setFlag, value)
{
	if(setFlag == true)
	{
		if (value == null || value == '')
		...
		if(unit_value == "hr" || unit_value == "hrs" || unit_value == "h" || unit_value == "hour" || unit_value == "hours")
		{
			value = int_value*3600000;
		}
		else if(unit_value == "min" || unit_value == "mins" || unit_value == "m" || unit_value == "minute" || unit_value == "minutes")
		{
			value = int_value*60000;
		}
		else if(unit_value == "sec" || unit_value == "secs" || unit_value == "s" || unit_value == "second" || unit_value == "seconds")
		{
		...
		setTimeout('pauseTimer()',value);setTimeout('quickBreakNotification()',value);
	}
	else
	{
		return;
...
function releaseMana(decrementValue) {
	var manabarElement = $( "#manabar" ),
      manaLabel = $( ".manabar-label" );
	var currentValue = manabarElement.progressbar( "value" );
	var newValue = currentValue;
	if(decrementValue <= currentValue)
		{
	...
	manabarElement.progressbar({      
      change: function() {
        manaLabel.text( manabarElement.progressbar( "value" ));
      }
    });
}

function updateMana(value)
{
	$("#manabar").progressbar({
      value: value
    }); 
...
$( document ).ready(function() {
    ...
    $("#manabar").bind('progressbarchange', function(event, ui) {
