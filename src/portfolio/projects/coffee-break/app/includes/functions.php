...
/* DISPLAY FUNCTIONS */
		...
		*/
		...
		while($countRow = mysql_fetch_row($countQuery))
		{
			if($countRow[0] > 0)
			{
				...
				$workHoursQuery = mysql_query("SELECT zero FROM work_week WHERE email = '$local_user_email' ");
				$breakHoursQuery = mysql_query("SELECT zero FROM break_week WHERE email = '$local_user_email' ");
				while($wrow = mysql_fetch_row($workHoursQuery))
				{	
					$numberOfHrsWorkedToday = $wrow[0];
				} 

				while($brow = mysql_fetch_row($breakHoursQuery))
				{	
					$numberOfHrsBreakedToday = $brow[0];
				}

				$_SESSION['USER_WORK_HRS_TODAY'] = $numberOfHrsWorkedToday;
				$_SESSION['USER_BREAK_HRS_TODAY'] = $numberOfHrsBreakedToday;
		...
		<script type="text/javascript">
			window.userLevel = <?php echo $_SESSION["LEVEL_OF_THE_SESSION"]; ?>
		</script>
		<?php
		echo "<tr>";
		echo "<td>";
	...
	function showManaBar($pageContext)
	{
		...
		if($_SESSION["LOGGED_IN"] == "TRUE")
		{
			...
			echo "<div id='dialog-manabar-reclaim' class='ui-helper-hidden'> <p> <label for='manabar-reset-amount'>Setting Mana to</label> <input type='text' id='manabar-reset-amount' style='border:0; color:#f6931f; font-weight:bold;'></p><div id='manabar-slider'></div></div>";
		}
	}
	...
	function showOptions($pageContext)
	{
		...
		switch ($pageContext) {
		    		...
		    		<!--Timed Break jQuery UI Dialog-->
		    		<div id='dialog-timed-break' class='ui-helper-hidden'>
		    			<p>
		    				<label for='timed-break-input'>
		    					Please enter the number of hours/mins/secs followed by a space and the unit after which you want to take a break. For example you can enter: 1 h or 1 hr or 10 m or 10 mins. (Valid units are - h/hr/hrs/hour/hours, m/min/mins/minute/minutes, s/sec/secs/second/seconds )
		    				</label>
		    				<input type='text' id='timed-break-input' style='border:0; color:#f6931f; font-weight:bold;' />		    					    					    			
		    			</p>
		    		</div>
		    		<div id='dialog-timed-break-knob' class='ui-helper-hidden'>
		    			<p>
		    				<table>
		    					<tr>
		    						<td><label for='timed-break-hour'>Hours</label><input type="text" value="1" id='timed-break-hour' class="dial-hour" data-cursor="true" /></td>
		    						<td><label for='timed-break-min'>Minutes</label><input type="text" value="0" id='timed-break-min' class="dial-min" data-cursor="true" /></td>
		    						<!--<tr>
		    							<td>Hour</td><td>Minutes</td>
		    						</tr>-->
		    					</tr>
		    				</table>	    					    			
		    			</p>
					...
					<?php
					$settingsTabsArray = array("general"=>"General","settings"=>"App Settings","events"=>"Event Log","notifications"=>"Notifications","notes"=>"Notes");
					$settingTabsIconsArray = array("general"=>"icon-edit","settings"=>"icon-cogs","events"=>"icon-list-alt","notifications"=>"icon-bell-alt","notes"=>"icon-file");
					foreach ($settingsTabsArray as $key => $value)						
					{
						if($key == $_SESSION['SETTINGS_MENU_ACTIVE'])
						{
							...
							{
								echo "<li class='active'>"."<a href='$notes'><i class='icon-fixed-width ".$settingTabsIconsArray["$key"]."'></i> $value</a>"."</li>";									
							}
							else
							{
						...
						else
						{
							...
							{
								echo "<li>"."<a href='$notes'><i class='icon-fixed-width ".$settingTabsIconsArray["$key"]."'></i> $value</a>"."</li>";									
							}
							else
							{
