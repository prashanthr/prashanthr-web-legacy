<?php
	...
	$preComputedHours = (float) 0.00;
	$computedHours = (float) 0.00;
	...
	if(!isset($_SESSION['session']))
	{
		...
		$_SESSION['todayworkhrs'] = (float) $_SESSION['USER_WORK_HRS_TODAY'];
		$_SESSION['todayworkhrs_last'] = (float) $_SESSION['USER_WORK_HRS_TODAY'];
		$_SESSION['todaybreakhrs'] = (float) $_SESSION['USER_BREAK_HRS_TODAY'];
		//Trying to handle a cumulative work hr computation for single day
		$preComputedHours = (float) $_SESSION['USER_WORK_HRS_TODAY'];
	}
	else
	{
		//$preComputedHours = (float) 0.00;
		$_SESSION['todayworkhrs_last'] = (float) $_SESSION['todayworkhrs_last'] + (float) $_SESSION['USER_WORK_HRS_TODAY'];
	}

	
	//$preComputedHours = (float) $_SESSION['USER_WORK_HRS_TODAY'];
	echo "preComputedHours$preComputedHours";
	...
	if($_SESSION['event_happen'] == 1)
	{
		$computedHours = $hours + ($minutes/60) + ($seconds/(60*60));	
	}
	else
	{
		$computedHours = $preComputedHours + $hours + ($minutes/60) + ($seconds/(60*60));		
	}
	
	$_SESSION['lastComputedHours'] = (float) $computedHours;
	echo "computedHours$computedHours";
	...
	function updateRecord($workhrs,$breakhrs, $totw, $totb)
	{
		...
		if($_SESSION["LOGGED_IN"] == "TRUE")
		{
			...
			else
			{
				...
				//$mana_week_update_query = mysql_query("UPDATE mana_week SET zero='$mana' WHERE email='$email'");
	...
	{
		if($_SESSION['LOGGED_IN'] == "TRUE" && ($_SESSION['LOGIN_TYPE'] != "GUEST"))
		{
			$email = $_SESSION['USEREMAIL'];
			$scaleFactor = 2.0;
			...
			if(!isset($_SESSION['POINTS_ENGINE_STARTUP']))
			{
				...
				echo "[DATA]$constant-engine_not_started-currentis-$currentPoints-pointsofday-".$_SESSION['POINTS_OF_THE_DAY']."-newis-$newPoints-pointsofsession-".$_SESSION['POINTS_OF_THE_SESSION']."-[/DATA]";

			}
			else
			{
				$workvalue = (float)($_SESSION['todayworkhours']);
				$breakvalue = (float)($_SESSION['todaybreakhours']);
				if($breakvalue > 1.0 || $workvalue > 1.0)
				{
					$val = ($breakvalue > $workvalue ? $breakvalue : $workvalue);
					$newPoints = $_SESSION['POINTS_OF_THE_SESSION'] + $scaleFactor * $val;
				}
				else
				{
					$newPoints = $_SESSION['POINTS_OF_THE_SESSION'] + $scaleFactor * 1.0;
				}

				$_SESSION['POINTS_OF_THE_SESSION'] = $newPoints;
				echo "[DATA]$constant-engine_started-Points-pointsofday-".$_SESSION['POINTS_OF_THE_DAY']."-newis-$newPoints-pointsofsession-".$_SESSION['POINTS_OF_THE_SESSION']."-[/DATA]";				
			}
		...
		else
		{
			...
			echo "[DATA]USERNOTLOGGEDIN-pointsofsession-".$_SESSION['POINTS_OF_THE_SESSION']."-[/DATA]";
		}
	}
	...
	function setPoints($newPoints,$email)
	{
		...
		$l = $_SESSION["LEVEL_OF_THE_SESSION"];
		$setPointsAndLevelQuery = mysql_query("UPDATE users SET caffeine_points='$newPoints', caffeine_level='$l' WHERE email='$email'");
	}
